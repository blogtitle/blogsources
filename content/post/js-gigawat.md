...
draft: true
---
**NOTE**: This article assumes you have already watched [destroy all software's wat](https://www.destroyallsoftware.com/talks/wat). If you did not, watch it and come back afterwards.
...
What starts to be weird is empty values:
```js
!![]; // true
!!{}; // true
!!""; // false
```
But beware, things get messy pretty soon:
```js
[] == true;     // false
!![] == true;   // true
...
A programmer coming from old, strongly typed languages, might expect `a + b + c === c + b + a` to always be true, but it is not reliably so in the new, cool languages:
```js
a = 1/3;
b = 5/7;
c = 7/13;
...
### Why:
Floating point sums are not commutative due to [rounding](https://en.wikipedia.org/wiki/Floating-point_arithmetic#Representable_numbers,_conversion_and_rounding). The point of ECMAScript is that it is not so simple to make sure your numbers never turn to doubles, while in all strongly typed languages you can just declare them as some kind of `int`.
...
### Why:
It is wery well explained on [this stackoverflow comment](https://stackoverflow.com/a/36926905)
...
"1" + 1; // out: "11"
```
And this is mostly fine, it makes sense to use the type as the dominant one: strings can represent more things than numbers after all.

But then you find out that you can multiply a string by a number, and that it is not a shorthand for `String.prototype.repeat` but implicitly casts the string to a number and executes the multiplication:
```js
" 2" * 4; // out: 8     <- NOTE THE LEADING SPACE
"a" * 4;  // out: NaN
```

But even this... is mostly fine... I guess. This made it necessary to have explicit conversion helper functions to go back from string to number. It is true that it was possible to multiply by one to cast to number, but that made everything harder to read (more than usual) so they decided to provide two helper functions: `parseInt` and `parseFloat`.
...
For example, instead of doing something like a `for` loop over an array, use `forEach` or `map` functions to closure the s`**`t out of your core logic and make it more readable.
...
When the `map` method is called, it passes three parameters (not one) the function. This means that the actual arguments passed to `parseInt` are:
...
## Scoping
If you come from any C-like language you might be used to variable shadowing: when a variable declared within a certain scope has the same name as a variable declared in an outer scope. 
This can be used to prevent accidental assignments to outer scope variables and to use less names in the code to keep it more readable.
...
### Why:
The explaination can be found [on MDN](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/var#Description)

## This and that and ()=> and function
...
madness("this is an argument");           // param: this is an argument
madness({ name: "this is an object"});    // param: [object Object]
madness({ param: "this is a field" });    // param: this is a field
```
This was in the language, and still is, but is now discouraged. To put this in Brendan Eich [words](https://twitter.com/brendaneich/status/68001466471817216): "with violates lexical scope, making program analysis (e.g. for security) hard to infeasible."
...
```
It gets trickier when you stumble in properties:
```js
...
No Error is thrown. It is just lost in the stack.
### Why:
The `finally` block is always executed, it doesn't care if the `catch` block threw a new Error, because it is supposed to cleanup stuff, and it will behave as intended.
